...
from .harness import AuditedBackend
...
async def test_compaction_never_overrides_a_zero_output_legality_clamp():
    ...
    backend = AuditedBackend(ScriptedBackend.from_responses([]), 8_192)
    ...
    # a call".
    result = await compaction_auto(messages, backend, budget=budget)
    assert result is None
    assert all(call["input"] + call["max_tokens"] <= call["cw"] for call in backend.calls), backend.calls
