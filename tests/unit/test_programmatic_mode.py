...
@pytest.fixture
def backend():
    return ScriptedBackend.from_responses([ScriptedResponse(text="ok")])
...
class TestToolSelection:
    def test_default_includes_all_builtins(self, backend, tmp_path):
    ...
    def test_programmatic_excludes_external_tools(self, backend, tmp_path):
        agent = AlanCodeAgent(
    ...
    def test_disabled_tools_filters(self, backend, tmp_path):
        agent = AlanCodeAgent(
            backend=backend, cwd=str(tmp_path),
    ...
    def test_explicit_tools_replaces_default(self, backend, tmp_path):
    ...
    def test_extra_tools_still_appended(self, backend, tmp_path):
        from tests.conftest import EchoTool
        agent = AlanCodeAgent(
            backend=backend, cwd=str(tmp_path),
            programmatic=True,
    ...
    def test_disabled_tools_composes_with_extra(self, backend, tmp_path):
        from tests.conftest import EchoTool
        agent = AlanCodeAgent(
            backend=backend, cwd=str(tmp_path),
            disabled_tools=["Echo"],
